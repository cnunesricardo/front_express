...
import { fetchProducts, apiUrlProducts } from 'src/app/utils/api'
...
<main className={`container mx-auto max-w-screen-lg min-h-screen ${theme === 'dark' ? 'bg-stone-600 text-white' : 'bg-white'}`}>
        ...
        <p className='text-2xl mt-9 mb-2 text-center font-bold underline'> Product List:</p>

        <div className="mb-16 container mx-auto max-w-lg">        
        <ul className="mt-2 pb-10 flex justify-items-left grid grid-cols-1 place-content-center gap-2">
        {products.map((product) =>(
            <li key={product.id}>                
              <div className="">
                    <h1 className='text-xl'>Id {product.id} - {product.title}</h1>
                </div>
            </li>
        ))}
        </ul>       
